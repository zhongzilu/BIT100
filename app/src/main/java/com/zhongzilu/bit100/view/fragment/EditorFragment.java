...
public class EditorFragment extends BaseFragment implements IEditorFragmentView, View.OnClickListener {
    ...
    public void onCreateAfter(Bundle savedInstanceState) {
        ...
        if (fileTemp == null) {
            Toast.makeText(App.getAppContext(), "路径参数有误！", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void shareMenu() {
        ...
        if (mName.getText().toString().isEmpty()) {
            App.showSnackbar(mContent, "当前标题为空");
            return;
        }
        if (mContent.getText().toString().isEmpty()) {
            App.showSnackbar(mContent, "当前内容为空");
            return;
        }
